...
export class StandardManagementComponent implements OnInit {
  ...
  getStrengthLevelsFormGroup(categoryIndex: number) {
    return this.getWeightCategoryFormGroup(categoryIndex).get(
      'strengthLevels'
    ) as FormGroup;
  }
  ...
  onSubmit(): void {
    ...
    if (this.editMode && this.currentStandardId) {
      this.standardService
        .updateStandard(this.currentStandardId, standardData)
        .subscribe({
          next: () => {
  ...
  editStandard(standard: Standard): void {
    ...
    standard.weightCategories.forEach((category) => {
      const categoryGroup = this.formBuilder.group({
        ...
        strengthLevels: this.formBuilder.group({
          beginner: [
            category.strengthLevels.beginner,
            [Validators.required, Validators.min(0)],
          ],
          novice: [
            category.strengthLevels.novice,
            [Validators.required, Validators.min(0)],
          ],
          intermediate: [
            category.strengthLevels.intermediate,
            [Validators.required, Validators.min(0)],
          ],
          advanced: [
            category.strengthLevels.advanced,
            [Validators.required, Validators.min(0)],
          ],
          elite: [
            category.strengthLevels.elite,
            [Validators.required, Validators.min(0)],
          ],
        }),
      });
