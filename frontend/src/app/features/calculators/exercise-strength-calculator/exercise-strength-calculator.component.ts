  ...
  onSubmit(): void {
    ...
    this.calculatorService.calculate1RM(weight, reps).subscribe({
      next: (oneRMData) => {
        ...
        this.calculatorService.calculateStrengthLevel(exercise, oneRMData.oneRM, bodyWeight, gender).subscribe({
          next: (strengthData) => {
            this.result = {
              ...strengthData,
          ...
          error: (error) => {
            this.error = error.error.message || "Strength level calculation failed"
            this.loading = false
          },
        })
      },
