...
export class StandardsViewComponent implements OnInit {
  ...
  loadData(): void {
    ...
    Promise.all([this.exerciseService.getExercises().toPromise(), this.standardService.getStandards().toPromise()])
      .then(([exercisesData, standardsData]) => {
        this.exercises = (exercisesData || []).filter((exercise) => exercise.isPowerlifting)
  ...
  onSubmit(): void {
    ...
    const standard = this.standards.find((s) => s.exercise._id === exerciseId && s.gender === gender)
    ...
    const weightCategory = standard.weightCategories.find(
      (category) => this.userWeight >= category.minWeight && this.userWeight <= category.maxWeight,
    )
    ...
    const { beginner, novice, intermediate, advanced, elite } = weightCategory.strengthLevels
    ...
    if (this.userOneRM < beginner) {
      ...
      this.userLevel = "beginner"
      this.userPercentage = 100 + ((this.userOneRM - beginner) / (novice - beginner)) * 100
    } else if (this.userOneRM < intermediate) {
      this.userLevel = "novice"
      this.userPercentage = ((this.userOneRM - novice) / (intermediate - novice)) * 100
    } else if (this.userOneRM < advanced) {
      this.userLevel = "intermediate"
      this.userPercentage = ((this.userOneRM - intermediate) / (advanced - intermediate)) * 100
    } else if (this.userOneRM < elite) {
      this.userLevel = "advanced"
      this.userPercentage = ((this.userOneRM - advanced) / (elite - advanced)) * 100
    } else {
